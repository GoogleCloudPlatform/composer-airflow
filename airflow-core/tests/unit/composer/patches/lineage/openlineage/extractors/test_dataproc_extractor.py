...
from airflow.composer.patches.lineage.openlineage.extractors.dataproc_extractor import DataprocExtractor
from airflow.providers.common.compat.openlineage.facet import Dataset
from airflow.providers.google.cloud.operators.dataproc import DataprocSubmitJobOperator
from airflow.providers.openlineage.extractors.base import OperatorLineage

INPUTS = [Dataset(namespace="dataproc_metastore", name="inputtable")]
OUTPUTS = [Dataset(namespace="dataproc_metastore", name="outputtable")]
...
class TestDataprocExtractor:
    ...
    @mock.patch(
        "airflow.composer.patches.lineage.openlineage.extractors.dataproc_extractor.DataprocSQLJobLineageExtractor",
        autospec=True,
    )
    @mock.patch(
        "airflow.composer.patches.lineage.openlineage.extractors.dataproc_extractor.DataprocHook",
        autospec=True,
    )
    @mock.patch("airflow.composer.patches.lineage.openlineage.extractors.utils.xcom_pull", autospec=True)
    ...
    @mock.patch("airflow.composer.patches.lineage.openlineage.extractors.utils.xcom_pull", autospec=True)
    @mock.patch(
        "airflow.composer.patches.lineage.openlineage.extractors.dataproc_extractor.log", autospec=True
    )
    ...
    @mock.patch(
        "airflow.composer.patches.lineage.openlineage.extractors.dataproc_extractor.xcom_pull", autospec=True
    )
    @mock.patch(
        "airflow.composer.patches.lineage.openlineage.extractors.dataproc_extractor.log", autospec=True
    )
    def test_extract_on_complete_job_id_not_found(
        self, mock_log, mock_xcom_pull, mock_dataproc_hook, mock_dataproc_sql_extractor
    ):
        operator = DataprocSubmitJobOperator(
            task_id=TEST_TASK_ID,
        ...
        mock_dataproc_sql_extractor.return_value.data_lineage.return_value = (INPUTS, OUTPUTS)
        task_instance = mock.MagicMock()
        mock_xcom_pull.return_value = "job-id"
        mock_dataproc_hook.return_value.get_job.side_effect = NotFound("Job not found.")
        ...
        mock_log.exception.assert_called_once_with(
            "The job with id %s wasn't found. Data lineage wasn't reported", "job-id"
        )
