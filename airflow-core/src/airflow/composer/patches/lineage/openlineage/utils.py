...
def _traverse_copy(original_dict, fields):
    """
    Return a (possibly shallow) copy of original_dict, containing only those given fields.
    ...
    for field, new_fields in fields.items():
        ...
        if new_fields == True:  # noqa: E712
            copy_dict[field] = _clean_schema_and_producer(original_dict[field])
        elif isinstance(original_dict[field], list):
            new_list = []
...
def get_redacted_event(event: dict) -> dict:
    ...
    FIELDS_TO_LOG = {
        ...
        "run": {"facets": {"gcp_composer_run": True}},
    }
