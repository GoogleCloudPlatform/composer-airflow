...
from airflow.composer.patches.lineage.openlineage.facets import (
    ComposerJobFacet,
    ComposerRunFacet,
    GcpLineageJobFacet,
    GcpOrigin,
)
from airflow.composer.patches.lineage.openlineage.utils import get_redacted_event, sanitize_display_name
...
class ComposerTransportConfig(Config):
    """Config Class for Composer Transport."""
...
class ComposerTransport(Transport):
    ...
    def emit(self, event: Event) -> None:
        try:
            ...
            log.info(str(get_redacted_event(event_dict)))

            # We use the openlineage SerDe module to send the event as a dictionary
            request = ProcessOpenLineageRunEventRequest({"parent": LOCATION_PATH, "open_lineage": event_dict})
    ...
    def _patch_event(self, event: Event) -> None:
        ...
        gcp_lineage_facet = GcpLineageJobFacet(
            ...
            origin=GcpOrigin(
                sourceType="COMPOSER",
                name=os.path.join(LOCATION_PATH, f"environments/{COMPOSER_ENVIRONMENT_NAME}"),
            ),
        )
    ...
    def _patch_bigquery_external_query_if_needed(self, event: Event) -> None:
        if event.run.facets.get("externalQuery") and event.run.facets["externalQuery"].source == "bigquery":
            if event.run.facets.get("bigQueryJob") is None:
                log.debug(
                    "run.facets.bigQueryJob facet not found when patching the BigQuery externalQuery facet"
                )
                return
            try:
