...
def _create_dagbag(dag_folder, queue):
    ...
    def _stringify(x):
        ...
        try:
            if x is None or type(x) in _primitive_types or (six.PY2 and type(x) is unicode):  # noqa
                return x
    ...
    if (
        os.path.isfile('/home/airflow/gcs/env_var.json') or
        os.path.isfile('/home/airflow/gcs/env_var.json.bkp')
    ):
        try:
