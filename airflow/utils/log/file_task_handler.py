...
class StreamTaskHandler(logging.StreamHandler):
    """
    Act as a log handler used by Composer.

    It exposes task logs in worker logs. This is required because fluentd is reading those logs to expose
...
class FileTaskHandler(logging.Handler):
    """
    Act as a python log handler.

    It handles and readstask instance logs. It creates and delegates log handling
    ...
    class ResilientFileHandler(logging.handlers.WatchedFileHandler, NonCachingRotatingFileHandler):
        """
        ...
        def emit(self, record):
            """Override WatchedFileHandler.emit to add error handling."""
            try:
                super().emit(record)
            except Exception:
        ...
        def handleError(self, record):
            """
            Override Handler.handleError to avoid logging log-related errors.
    ...
    def _create_file_handler(self, local_loc) -> NonCachingRotatingFileHandler | ResilientFileHandler:
        return (
            FileTaskHandler.ResilientFileHandler(local_loc)
            if self._use_resilient_file_handler()
            else NonCachingRotatingFileHandler(local_loc, encoding="utf-8")
        )
