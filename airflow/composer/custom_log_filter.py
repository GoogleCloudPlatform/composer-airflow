...
PROVIDERS_HOOK_MISSING_ATTRIBUTE_WARNING_RE = re2.compile(
...
def _is_stats_client_warning(record):
    """Detect stats client is not configured warning."""
...
def _is_refused_to_delete_permission_view_warning(record):
    """Detect refused to delete permission view warning."""
...
def _is_no_user_yet_created_warning(record):
    """Detect no user yet created warning."""
...
def _is_providers_hook_missing_attribute_warning(record):
    """Detect missing attribute warning for provider hooks."""
...
def _is_duplicate_key_value_in_permission_tables_warning(record):
    """Detect that detects postgres warning for violating unique constraint in permission tables."""
    constraints = [
...
def _is_flower_warning(record):
    """Detect warnings produced by flower, which are not actionable."""
    record_message = record.getMessage()
    return "Inspect method" in record_message and "failed" in record_message


def _is_scheduled_duration_metric_warning(record):
    """Detect warnings about scheduled_duration metric, which should not be emitted."""
...
def _is_unformatted_permission_tables_warning(record):
    """Detect unformatted postgres warning for updating permission tables."""
