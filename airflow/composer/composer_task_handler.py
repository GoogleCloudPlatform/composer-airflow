...
class ComposerTaskHandler(StreamTaskHandler, LoggingMixin):
    ...
    This handler reads logs for executing tasks from Cloud Logging.
    ...
    def __init__(
        ...
        scopes: Collection[str] | None = _DEFAULT_SCOPES,
    ...
    def read(
    ...
    ) -> tuple[list[tuple[tuple[str, str]]], list[dict[str, Any]]]:
        """
        ...
        """
        self.log.info("Reading logs with ComposerTaskHandler for %s", str(task_instance))
        messages = ""
        ...
        if not messages and next_page_token:
            return [()], [new_metadata]  # type: ignore

        return [((self.task_instance_hostname, messages),)], [new_metadata]

    def _prepare_filter(self, task_instance: TaskInstance, try_number: int | None = None) -> str:
        """
    ...
    def _read_logs(
        ...
        """
        Send requests to the Cloud Logging service and downloads logs.
    ...
    def _read_single_logs_page(self, log_filter: str, page_token: str | None = None) -> tuple[str, Any]:
        """
        Send requests to the Cloud Logging service and downloads single page with logs.
