...
class DataprocSQLJobLineageExtractor:
    ...
    def data_lineage(self, output_type="dataset") -> tuple[
        list[DataprocMetastoreTable] | list[Dataset] | None,
        list[DataprocMetastoreTable] | list[Dataset] | None,
    ]:
        """Extracts SQL queries from the Dataproc job and generates data lineage.

        Returns:
            Tuple consisting of two lists of Dataset or DataprocMetastoreTable.
        ...
        elif output_type == "metastore_table":
            return self._build_lineage_entities_as_metastore_table(
                tables=source_tables
            ), self._build_lineage_entities_as_metastore_table(tables=target_tables)
        else:
    ...
    def _build_lineage_entities_as_metastore_table(
        self, tables: list[ParsedSQLTable]
    ) -> list[DataprocMetastoreTable]:
