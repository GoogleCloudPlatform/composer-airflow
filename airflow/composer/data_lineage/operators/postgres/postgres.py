...
class PostgresOperatorLineageMixin:
    ...
    def post_execute_prepare_lineage(self: PostgresOperator, context: dict):  # type: ignore
        ...
        for query in sql_queries:
            lineage_runner = LineageRunner(sql=query, dialect="postgres")
            try:
            ...
            if source_tables and target_tables:
                try:
                    inlets = _map_to_postgres_tables(source_tables, db_default, host, port)  # type: ignore
                    outlets = _map_to_postgres_tables(target_tables, db_default, host, port)  # type: ignore
...
def _map_to_postgres_tables(lineage_tables: list, db_default: str, host: str, port: str) -> list:
    ...
    for lineage_table in lineage_tables:
        ...
        result_tables.append(
            PostgresTable(
                host=host, port=port, database=database, schema=schema, table=lineage_table.raw_name
            )
        )
