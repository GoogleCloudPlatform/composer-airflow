...
class TestCliTasks:
    ...
    def test_cli_run_wait_dag_not_found_timeout(self, get_dag_mock):
        ...
        with pytest.raises(
            AirflowException,
            match="Dag 'example_python_operator' could not be found",
        ), conf_vars({("core", "wait_dag_not_found_timeout"): "60", ("core", "load_examples"): "False"}):
            task_command.task_run(
                self.parser.parse_args(
                    ["tasks", "run", self.dag_id, self.dag.task_ids[0], "--local", self.run_id]
                )
            )

        get_dag_mock.assert_called_with(mock.ANY, self.dag_id, wait_dag_not_found_timeout=60)
