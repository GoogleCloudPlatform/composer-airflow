...
class TestComposerMigrations:
    ...
    def test_task_instance_index(self):
        with create_session() as session:
            ...
            assert any(index["name"] == "ti_worker_healthcheck" for index in indices)

        assert any(index.name == "ti_worker_healthcheck" for index in TaskInstance.__table_args__)
