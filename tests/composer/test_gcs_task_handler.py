...
class TestGCSTaskHandler(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.dag.create_dagrun(state=State.SUCCESS, run_id="test_run_id", execution_date=date)
    ...
    def test_should_read_logs_from_remote(self, mock_blob, mock_client, mock_creds):
        ...
        mock_blob.from_string.assert_called_once_with(
            "gs://bucket/remote/log/location/dag_id=dag_for_testing_task_handler/run_id=test_run_id/task_id=task_for_testing_gcs_task_handler/attempt=1.log", mock_client.return_value
        )

        self.assertEqual(
            "*** Reading remote log from gs://bucket/remote/log/location/dag_id=dag_for_testing_task_handler/run_id=test_run_id/task_id=task_for_testing_gcs_task_handler/attempt=1.log.\nCONTENT\n", logs
        )
    ...
    def test_gcs_not_found_file(self, mock_blob, mock_client, mock_creds):
        ...
        self.assertEqual(
            log,
            (
                "*** Log file is not found: gs://bucket/remote/log/location/dag_id=dag_for_testing_task_handler/run_id=test_run_id/task_id=task_for_testing_gcs_task_handler/attempt=1.log.\n"
    ...
    def test_gcs_unknown_exception(self, mock_blob, mock_client, mock_creds):
        ...
        self.assertEqual(
            log,
            "*** Unable to read remote log from gs://bucket/remote/log/location/dag_id=dag_for_testing_task_handler/run_id=test_run_id/task_id=task_for_testing_gcs_task_handler/attempt=1.log\n*** "
            "Failed to connect",
        )
        self.assertDictEqual(metadata, {"end_of_log": True})
        mock_blob.from_string.assert_called_once_with(
    ...
    def test_write_to_remote_on_close(self, mock_blob, mock_client, mock_creds):
        ...
        mock_blob.assert_has_calls(
            [
                mock.call.from_string("gs://bucket/remote/log/location/dag_id=dag_for_testing_task_handler/run_id=test_run_id/task_id=task_for_testing_gcs_task_handler/attempt=1.log", mock_client.return_value),
    ...
    def test_failed_write_to_remote_on_close(self, mock_blob, mock_client, mock_creds):
        ...
        self.assertEqual(
            ...
            [
                ...
                'not write logs to gs://bucket/remote/log/location/dag_id=dag_for_testing_task_handler/run_id=test_run_id/task_id=task_for_testing_gcs_task_handler/attempt=1.log: Failed to connect',
            ],
        )
        mock_blob.assert_has_calls(
            [
