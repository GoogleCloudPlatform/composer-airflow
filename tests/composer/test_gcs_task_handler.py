...
class TestGCSTaskHandler:
    ...
    def test_failed_write_to_remote_on_close(self, mock_blob, mock_client, mock_creds, caplog):
        ...
        assert (
            "airflow.composer.gcs_task_handler.GCSTaskHandler",
            logging.INFO,
            "Previous log discarded: sequence item 0: expected str instance, bytes found",
        ) == caplog.record_tuples[0]
        log = (
            "Could not write logs to gs://bucket/remote/log/location/dag_id=dag_for_testing_task_handler/run_id=test_run_id"
            "/task_id=task_for_testing_gcs_task_handler/attempt=1.log: Failed to connect"
        )
        assert (
            "airflow.composer.gcs_task_handler.GCSTaskHandler",
            logging.ERROR,
