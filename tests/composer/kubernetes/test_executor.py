...
class TestExecutor:
                        ...
                        spec=V1PodSpec(
                            affinity=V1Affinity(
                                node_affinity=V1NodeAffinity(
                                    preferred_during_scheduling_ignored_during_execution=V1PreferredSchedulingTerm(
                                        preference={},
                                        weight=100,
                                    ),
                                )
    ...
    def test_patch_fetch_container_logs(
        self,
        _composer_patch_kubernetes_executor_start_mock,
        _composer_patch_kubernetes_executor_init_mock,
    ):
        # test setUp
    ...
    def test_patch_kubernetes_executor_init_and_sync_with_event_scheduler(
        self,
        _composer_patch_kubernetes_executor_sync_mock,
        event_scheduler_mock,
    ):
    ...
    @mock.patch("airflow.composer.kubernetes.executor.EventScheduler", autospec=True)
    def test_composer_kubernetes_executor_sync(self, event_scheduler_mock):
