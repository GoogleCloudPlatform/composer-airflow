...
class TestPostgresOperator(unittest.TestCase):
    ...
    def test_post_execute_prepare_lineage_sql_insert_with_source_target_tables(self, mock_get_uri):
        ...
        self.assertEqual(
            ...
            [
                PostgresTable(
                    host=HOST, port=PORT, database=DATABASE, schema="public", table="source_table"
                )
            ]
        )
        self.assertEqual(
            ...
            [
                PostgresTable(
                    host=HOST, port=PORT, database=DATABASE, schema="public", table="target_table"
                )
            ]
        )
    ...
    @patch("airflow.composer.data_lineage.operators.postgres.postgres.PostgresHook", autospec=True)
