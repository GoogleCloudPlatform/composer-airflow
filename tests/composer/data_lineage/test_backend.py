...
from google.cloud.datacatalog_lineage_v1 import LineageEvent, Process, Run
...
class TestBackend:
    ...
    def test_send_lineage(self, mock_composer_data_lineage_adapter, mock_sync_lineage_client):
        ...
        def _mock_get_lineage_events_bundle_on_task_completed(task_instance, inlets, outlets):
            assert task_instance == mock_ti
            assert inlets == mock_inlets
            assert outlets == mock_outlets
        ...
        assert mock_sync_lineage_client().create_events_bundle.call_args_list[0][1]["retry"]._deadline == 5
