...
@pytest.mark.parametrize("operator", [GCSToBigQueryOperator, GoogleCloudStorageToBigQueryOperator])
class TestGCSToBigQuery:
    @pytest.mark.parametrize(
        "test_id, source_objects, destination, expected_inlets, expected_outlets",
        [
            (
    ...
    def test_post_execute_prepare_lineage(
        self, mock_hook, test_id, source_objects, destination, expected_inlets, expected_outlets, operator
    ):
    ...
    @patch.object(BigQueryHook, "project_id", new_callable=PropertyMock)
    def test_post_execute_prepare_lineage_split_table_name_error(self, mock_hook, operator):
