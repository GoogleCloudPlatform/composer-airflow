...
class TestMySQLToGCSOperator:
    @patch.object(MySqlHook, "get_uri")
    def test_post_execute_prepare_lineage_sql_simple(self, uri_mock, operator):
        ...
        assert task.inlets == [MySQLTable(host=HOST, port=PORT, schema=SCHEMA, table=TEST_TABLE)]
        assert task.outlets == [GCSEntity(bucket=BUCKET, path=FILENAME)]

    @patch.object(MySqlHook, "get_uri")
    def test_post_execute_prepare_lineage_sql_with(self, uri_mock, operator):
        uri_mock.return_value = f"mysql://user:***@{HOST}:{PORT}/{SCHEMA}"
    ...
    @patch.object(MySqlHook, "get_uri")
    def test_post_execute_prepare_lineage_sql_joins(self, uri_mock, operator):
        uri_mock.return_value = f"mysql://user:***@{HOST}:{PORT}/{SCHEMA}"
    ...
    @patch.object(MySqlHook, "get_uri")
    def test_post_execute_prepare_lineage_default_port(self, uri_mock, operator):
        ...
        assert task.inlets == [MySQLTable(host=HOST, port=DEFAULT_PORT, schema=SCHEMA, table=TEST_TABLE)]
        assert task.outlets == [GCSEntity(bucket=BUCKET, path=FILENAME)]

    @patch.object(MySqlHook, "get_uri")
    def test_post_execute_prepare_lineage_no_sources(self, uri_mock, operator):
        uri_mock.return_value = f"mysql://user:***@{HOST}:{PORT}/{SCHEMA}"

        task = operator(task_id="test-task", sql=SQL_WITHOUT_SOURCES, bucket=BUCKET, filename=FILENAME)
    ...
    @patch.object(MySqlHook, "get_uri")
    def test_post_execute_prepare_lineage_connection_error(self, uri_mock, operator):
    ...
    @patch.object(MySqlHook, "get_uri")
    def test_post_execute_prepare_lineage_sql_parsing_error(self, uri_mock, operator):
        uri_mock.return_value = f"mysql://user:***@{HOST}:{PORT}/{SCHEMA}"

        task = operator(task_id="test-task", sql=SQL_ERROR, bucket=BUCKET, filename=FILENAME)
    ...
    @patch.object(MySqlHook, "get_uri")
    def test_post_execute_prepare_lineage_sqlparse_type_error(self, uri_mock, operator):
        uri_mock.return_value = f"mysql://user:***@{HOST}:{PORT}/{SCHEMA}"

        task = operator(task_id="test-task", sql=None, bucket=BUCKET, filename=FILENAME)
    ...
    @patch.object(MySqlHook, "get_uri")
    def test_post_execute_prepare_lineage_sql_multiple_queries(self, uri_mock, operator):
        uri_mock.return_value = f"mysql://user:***@{HOST}:{PORT}/{SCHEMA}"
        ...
        assert task.inlets == [MySQLTable(host=HOST, port=PORT, schema=SCHEMA, table=SOURCE_TABLE_1)]
        assert task.outlets == [GCSEntity(bucket=BUCKET, path=FILENAME)]
    ...
    @patch.object(MySqlHook, "get_uri")
    def test_post_execute_prepare_lineage_sql_no_select(self, uri_mock, test_id, sql, operator):
        uri_mock.return_value = f"mysql://user:***@{HOST}:{PORT}/{SCHEMA}"
        task = operator(task_id="test-task", sql=sql, bucket=BUCKET, filename=FILENAME)
