...
@pytest.mark.parametrize("operator", [BigQueryToGCSOperator, BigQueryToCloudStorageOperator])
class TestBigQueryToGCS:
    @patch.object(BigQueryHook, "project_id", new_callable=PropertyMock)
    def test_post_execute_prepare_lineage_single_target(self, mock_project_id, operator):
        task = operator(
            task_id=TASK_ID,
    ...
    @patch.object(BigQueryHook, "project_id", new_callable=PropertyMock)
    def test_post_execute_prepare_lineage_target_with_wildcard(self, mock_project_id, operator):
        task = operator(
            task_id=TASK_ID,
            source_project_dataset_table=f"{SOURCE_PROJECT_ID}.{SOURCE_DATASET_ID}.{SOURCE_TABLE_ID}",
    ...
    @patch.object(BigQueryHook, "project_id", new_callable=PropertyMock)
    def test_post_execute_prepare_lineage_multiple_targets(self, mock_project_id, operator):
        task = operator(
            task_id=TASK_ID,
            source_project_dataset_table=f"{SOURCE_PROJECT_ID}.{SOURCE_DATASET_ID}.{SOURCE_TABLE_ID}",
    ...
    @patch.object(BigQueryHook, "project_id", new_callable=PropertyMock)
    def test_post_execute_prepare_lineage_parsing_tablename_inlet_error(self, mock_project_id, operator):
    ...
    @patch.object(BigQueryHook, "project_id", new_callable=PropertyMock)
    def test_post_execute_prepare_lineage_parsing_uri_outlet_error(self, mock_project_id, operator):
        mock_project_id.return_value = SOURCE_PROJECT_ID
