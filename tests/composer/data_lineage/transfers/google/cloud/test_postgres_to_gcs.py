...
class TestPostgresToGCSOperator:
    @patch.object(PostgresHook, "get_uri")
    def test_post_execute_prepare_lineage_sql_single_source(self, mock_get_uri, operator):
        ...
        assert task.outlets == [GCSEntity(bucket=TARGET_BUCKET_NAME, path=TARGET_BUCKET_FILENAME)]

    @patch.object(PostgresHook, "get_uri")
    def test_post_execute_prepare_lineage_sql_with_subquery(self, mock_get_uri, operator):
        mock_get_uri.return_value = f"postgres://login:password@{HOST}:{PORT}/{DATABASE}"
        ...
        assert task.inlets == [
            PostgresTable(host=HOST, port=PORT, database=DATABASE, schema="public", table="source_table"),
            PostgresTable(host=HOST, port=PORT, database=DATABASE, schema="public", table="source_table_2"),
        ]
        assert task.outlets == [GCSEntity(bucket=TARGET_BUCKET_NAME, path=TARGET_BUCKET_FILENAME)]

    @patch.object(PostgresHook, "get_uri")
    def test_post_execute_prepare_lineage_sql_with_db_schema(self, mock_get_uri, operator):
        mock_get_uri.return_value = f"postgres://login:password@{HOST}:{PORT}/{DATABASE}"
    ...
    @patch.object(PostgresHook, "get_uri")
    def test_post_execute_prepare_lineage_sql_with_schema(self, mock_get_uri, operator):
        mock_get_uri.return_value = f"postgres://login:password@{HOST}:{PORT}/{DATABASE}"
    ...
    @patch.object(PostgresHook, "get_uri")
    def test_post_execute_prepare_lineage_default_port(self, mock_get_uri, operator):
    ...
    @patch.object(PostgresHook, "get_uri")
    def test_post_execute_prepare_lineage_sql_no_source_tables_error(self, mock_get_uri, operator):
        mock_get_uri.return_value = f"postgres://login:password@{HOST}:{PORT}/{DATABASE}"
    ...
    @patch.object(PostgresHook, "get_uri")
    def test_post_execute_prepare_lineage_parsing_db_schema_parts_error(self, mock_get_uri, operator):
        mock_get_uri.return_value = f"postgres://login:password@{HOST}:{PORT}/{DATABASE}"
    ...
    @patch.object(PostgresHook, "get_uri")
    def test_post_execute_prepare_lineage_sql_parsing_incorrect_query_error(self, mock_get_uri, operator):
        mock_get_uri.return_value = f"postgres://login:password@{HOST}:{PORT}/{DATABASE}"
    ...
    @patch.object(PostgresHook, "get_uri")
    def test_post_execute_prepare_lineage_connection_error(self, mock_get_uri, operator):
    ...
    @patch.object(PostgresHook, "get_uri")
    def test_post_execute_prepare_lineage_sqlparse_type_error(self, mock_get_uri, operator):
        mock_get_uri.return_value = f"postgres://login:password@{HOST}:{PORT}/{DATABASE}"
    ...
    @patch.object(PostgresHook, "get_uri")
    def test_post_execute_prepare_lineage_sql_multiple_queries(self, mock_get_uri, operator):
        mock_get_uri.return_value = f"postgres://login:password@{HOST}:{PORT}/{DATABASE}"
    ...
    @patch.object(PostgresHook, "get_uri")
    def test_post_execute_prepare_lineage_sql_no_select(self, mock_get_uri, test_id, sql, operator):
        mock_get_uri.return_value = f"postgres://login:password@{HOST}:{PORT}/{DATABASE}"
        task = operator(
