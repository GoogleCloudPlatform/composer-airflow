...
class TestTimeSensorAsync:
    ...
    def test_task_is_deferred_when_triggerer_enabled(self):
        with DAG(
            "test_task_is_deferred",
            schedule=None,
            start_date=timezone.datetime(2020, 1, 1, 23, 0),
        ):
            op = TimeSensorAsync(task_id="test", target_time=time(10, 0))

        with patch("airflow.configuration.conf.getboolean", autospec=True) as mock:

            def mock_get_boolean(section, key, **kwargs):
                return section == "composer_internal" and key == "enable_triggerer"

            mock.side_effect = mock_get_boolean
            with pytest.raises(TaskDeferred) as exc_info:
                op.execute({})

            mock.assert_called_once_with(section="composer_internal", key="enable_triggerer", fallback=False)
